...
for dataset in range(3):
    ...
    d=a.obs['ape']
    print "Start Point", d['lon'][0],d['lat'][0]
...
#        ax1=fig.add_subplot(121)
    ...
    #    ax1.plot(a3.obs['ape']['lon'][:],-1*a3.obs['ape']['lat'][:],'k-')
        ...
        for tail in range(max(min(koko,i),1)-1,-1,-1):
            ...
            plt.plot(x[i-tail],y[i-tail],'ro',markersize=koko-tail,alpha=1.0-float(tail)/koko)
            """
 ...
 #       plot_full_data(set_no=dataset,high_no=i,value="temp")
        ...
        fig.colorbar(im)
        """
        plt.savefig('Map_%.3d.png' % (i+offset),facecolor=fig.get_facecolor(),dpi=120)
