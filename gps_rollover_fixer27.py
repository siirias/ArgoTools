...
def fix_stamps_for_text(in_data, time_formats=None, gps_time_fix=None):
    ...
    if(not gps_time_fix):
        gps_time_fix = dt.timedelta(seconds = 7*24*60*60*1024.0)
    if(not time_formats):
...
def main(argv):
    ...
    try:
        ...
        print 'gps_rollover_fixer.py -i <inputfile> -b <backupfile>'
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
    ...
    if not os.path.exists(inputfile):
        print 'file does not exist: %s' % inputfile
        sys.exit(1)
